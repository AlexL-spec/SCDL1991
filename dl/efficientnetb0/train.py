# src/dl/efficientnetb0/train.py
import argparse
import yaml
...
def build_custom_efficientnetb0_model(num_classes, img_size_hw=(224, 224), base_trainable_setting=20,
                                      dropout_rate=0.3):
    input_shape_with_channels = (*img_size_hw, 3)
    base_model_for_build = EfficientNetB0(weights="imagenet", include_top=False,
                                          input_shape=input_shape_with_channels, pooling='avg')

    if isinstance(base_trainable_setting, bool):
        base_model_for_build.trainable = base_trainable_setting
        print(f"  EfficientNetB0 base_model_for_build.trainable set to: {base_trainable_setting}")
    elif isinstance(base_trainable_setting, int) and base_trainable_setting >= 0:
        if base_trainable_setting == 0:
            print(f"  Freezing all layers of EfficientNetB0 base_model_for_build.")
            base_model_for_build.trainable = False
        elif len(base_model_for_build.layers) >= base_trainable_setting:
            print(f"  Unfreezing last {base_trainable_setting} layers of EfficientNetB0 base_model_for_build.")
            for layer in base_model_for_build.layers: layer.trainable = False  # Freeze all first
            for layer in base_model_for_build.layers[-base_trainable_setting:]: layer.trainable = True
        else:
            print(
                f"  Warning: base_trainable_setting ({base_trainable_setting}) >= num layers in EfficientNetB0 base_model_for_build ({len(base_model_for_build.layers)}). Unfreezing all base_model_for_build layers.")
            base_model_for_build.trainable = True
    else:
        print(
            f"  Invalid base_trainable_setting: {base_trainable_setting}. Freezing all layers of EfficientNetB0 base_model_for_build.")
        base_model_for_build.trainable = False
    ...
    model = Model(inputs=inputs, outputs=outputs, name="CellTypeEfficientNetB0")
    return model
...
def get_preprocessing_function(cnn_model_name_str: str, target_colorspace_str: str):
    ...
    def custom_image_processor(image_np_rgb):
        ...
        elif processed_image_intermediate.ndim == 3 and processed_image_intermediate.shape[
            2] == 1:
            rescaled_channel = rescale_intensity(processed_image_intermediate[:, :, 0], out_range=(0, 255))
            final_output_image = np.dstack((rescaled_channel, rescaled_channel, rescaled_channel))
        else:
            print(
                f"Warning: Unexpected image shape {processed_image_intermediate.shape} after color conversion. Attempting direct scaling to [0,255].")
...
def train_single_fold(fold_num: int, run_cfg: dict, cnn_model_name: str = "efficientnetb0"):
    ...
    hparams_cnn.setdefault('base_trainable_setting',
                           hparams_all.get(cnn_model_name, {}).get('base_trainable_setting', 20))
    hparams_cnn.setdefault('dropout', hparams_all.get(cnn_model_name, {}).get('dropout', 0.3))
    ...
    model = build_custom_efficientnetb0_model(
    ...
    if val_generator.n > 0:
        ...
        report_target_names = [idx_to_class[i] for i in sorted(idx_to_class.keys())]  # Ensure correct order
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train an EfficientNetB0 model for a specific fold/run configuration.")
    ...
    try:
        train_single_fold(fold_num=fold_to_validate_on, run_cfg=run_config, cnn_model_name="efficientnetb0")
    except Exception as e:
        print(f"FATAL ERROR during train_single_fold for efficientnetb0, ValFoldNum {fold_to_validate_on}: {e}")
        import traceback

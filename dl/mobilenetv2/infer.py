...
def preprocess_for_inference(
        ...
        cnn_model_name: str = "mobilenetv2"
):
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch/single-image inference for trained MobileNetV2 models.")
    ...
    parser.add_argument("--output_csv", default="predictions_mobilenetv2.csv", type=Path,
    ...
    try:
        ...
        print(f"MobileNetV2 model loaded from {args.model_path}")
    ...
    print(f"Found {len(image_files)} images to predict using MobileNetV2.")
    predictions_data = []
    target_size = (args.img_height, args.img_width)

    for img_file in tqdm(image_files, desc="Predicting images (MobileNetV2)"):
        try:
    ...
    try:
        ...
        print(f"MobileNetV2 predictions saved to {args.output_csv}")
    except Exception as e:

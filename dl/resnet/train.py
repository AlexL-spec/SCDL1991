...
def build_custom_resnet_model(num_classes, img_size_hw=(224, 224), base_trainable_setting=10, dropout_rate=0.5):
    input_shape_with_channels = (*img_size_hw, 3)

    base_model_for_build = ResNet50(
        include_top=False,
        weights="imagenet",
    ...
    if isinstance(base_trainable_setting, bool):
        ...
        print(f"  ResNet50 base_model_for_build.trainable set to: {base_trainable_setting}")
    elif isinstance(base_trainable_setting, int) and base_trainable_setting >= 0:
        if base_trainable_setting == 0:
            print(f"  Freezing all layers of ResNet50 base_model_for_build.")
            base_model_for_build.trainable = False
        elif len(base_model_for_build.layers) >= base_trainable_setting:
            print(f"  Unfreezing last {base_trainable_setting} layers of ResNet50 base_model_for_build.")
            ...
            print(
                f"  Warning: base_trainable_setting ({base_trainable_setting}) >= num layers in ResNet50 base_model_for_build ({len(base_model_for_build.layers)}). Unfreezing all base_model_for_build layers.")
            base_model_for_build.trainable = True
    else:
        print(
            f"  Invalid base_trainable_setting: {base_trainable_setting}. Freezing all layers of ResNet50 base_model_for_build.")
        base_model_for_build.trainable = False

    inputs = Input(shape=input_shape_with_channels, name="input_image")
    x = tf.keras.applications.resnet50.preprocess_input(inputs)

    is_any_layer_in_base_trainable = any(layer.trainable for layer in base_model_for_build.layers)
    x_base = base_model_for_build(x, training=is_any_layer_in_base_trainable)
    ...
    model = Model(inputs=inputs, outputs=outputs, name="CellTypeResNet50")
    return model
...
def get_preprocessing_function(cnn_model_name_str: str, target_colorspace_str: str):
    ...
    def custom_image_processor(image_np_rgb):
        ...
        if np.max(current_image_float) > 1.01:  # Heuristic for 0-255 scale
...
def train_single_fold(fold_num: int, run_cfg: dict, cnn_model_name: str = "resnet"):
    ...
    hparams_cnn.setdefault('base_trainable_setting',
                           hparams_all.get(cnn_model_name, {}).get('base_trainable_setting', 10))
    hparams_cnn.setdefault('dropout', hparams_all.get(cnn_model_name, {}).get('dropout', 0.5))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train a ResNet model for a specific fold/run configuration.")
    ...
    try:
        train_single_fold(fold_num=fold_to_validate_on, run_cfg=run_config, cnn_model_name="resnet")
    except Exception as e:
        print(f"FATAL ERROR during train_single_fold for resnet, ValFoldNum {fold_to_validate_on}: {e}")
        import traceback
